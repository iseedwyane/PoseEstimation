...
if __name__ == "__main__":
    CAP = MultiVideoCapture(cameraID_outside=6, cameraID_left=10, cameraID_right=0, cameraID_behind=8)
    ...
    initial_pose_path = "./IMG_DATA_LS/IMG_DATA_MASTERBALL_0905-183019/Pose_Init"
    ...
    while count < total_samples:
        ...
        result_img, pose_data = MarkerDetector.readPose(img_outside)
        ...
        if pose_data:
            ...
            if all(req_id in detected_ids for req_id in required_ids):
                ...
                if GroundTruthPOSE is not None:
                    ...
                    # 将 GroundTruthPOSE 转为二维数组（1行6列），确保拼接时形状正确
                ...
                for i in range(len(pose_data)):
                    if pose_data[i][0] == 12:
                        if GroundTruthPOSE is not None:
                            pose_data[i][1:7] = GroundTruthPOSE        

                # 保存姿态数据到 result_folder，使用八位数文件名
                file_name_txt = f"{count:08d}.txt"
                np.savetxt(os.path.join(img_folder, result_folder, file_name_txt), pose_data)

        file_name = f"{count:08d}.png"
    ...
    csv_filename = os.path.join(img_folder, 'resultpose.csv')  # resultpose.csv 存放在 img_folder
