...
from PoseTransfer_Raw import RawDataTransfer
...
class ReconstDataset(Dataset):
    def __init__(self, data_path="./IMG_DATA/train.txt", scaled_width = 320, scale_height= 320):
        ...
        for j in range(len(temp_img_list)):
            temp = os.path.join(path,"IMG_LEFT",temp_img_list[j])
            temp1 = temp.replace("IMG_LEFT", "IMG_LEFT")
            temp2 = temp.replace("IMG_LEFT", "IMG_RIGHT")
            temp3 = temp.replace("IMG_LEFT", "IMG_BEHIND")  # 新增的 behind 图像路径
            temp4 = temp.replace("IMG_LEFT", "POSE_TXT").replace('jpg', 'txt')
            ...
            if os.path.isfile(temp) == False or os.path.isfile(temp1) == False or os.path.isfile(temp2) == False or os.path.isfile(temp3) == False or os.path.isfile(temp4) == False:
                continue
            # check data format
    ...
    def __getitem__(self, item):
        ...
        # 左相机
        temp1 = img_path_abs.replace("IMG_LEFT", "IMG_LEFT")
        ...
        # 右相机
        temp2 = img_path_abs.replace("IMG_LEFT", "IMG_RIGHT")
        ...
        # 后方相机
        temp3 = img_path_abs.replace("IMG_LEFT", "IMG_BEHIND")
        img3_data = cv2.imread(temp3)
        img3_data = cv2.cvtColor(img3_data, cv2.COLOR_BGR2GRAY)
        img3_data = img3_data / 255.0
        img3_data = cv2.resize(img3_data, dsize=(self.width, self.height))

        # pose label
        temp4 = img_path_abs.replace("IMG_LEFT", "POSE_TXT")
        temp4 = temp4.replace('jpg','txt')
        joint_config, objectPose = self.PoseReader.run(temp4)
        #print('objectPose in ReconstDataset: ', objectPose.shape)


        # label_data = {'image': img_data}
        # return torch.Tensor([img1_data])
        #print("img1_tensor shape:", torch.Tensor([img1_data]).shape)  
        #print("img2_tensor shape:", torch.Tensor([img2_data]).shape)    
        #print("img3_tensor shape:", torch.Tensor([img3_data]).shape)        
        #print("joint_config shape:", torch.Tensor([joint_config]).shape)  
        #print("objectPose shape:", torch.Tensor([objectPose]).shape)  
        # return torch.from_numpy(img1_data), torch.from_numpy(img2_data), torch.from_numpy(joint_config), torch.from_numpy(objectPose)
        #print("ReconstDataset  return ====================================:")
        return torch.Tensor(np.array([img1_data])), torch.Tensor(np.array([img2_data])), torch.Tensor([img3_data]), torch.Tensor(np.array([joint_config])), torch.Tensor(np.array([objectPose]))
...
class ClassificationDataset(Dataset):
    def __init__(self, data_path="./IMG_DATA/", data_mode = "train",scaled_width = 320, scale_height= 320):
        ...
        for i in range(len(folder_list)):
            ...
            for j in range(len(temp_img_list)):
                temp = os.path.join(folder_list, "IMG_LEFT", temp_img_list[j])
                temp1 = temp.replace("IMG_LEFT", "IMG_LEFT")
                temp2 = temp.replace("IMG_LEFT", "IMG_RIGHT")
                temp3 = temp.replace("IMG_LEFT", "IMG_BEHIND")  # 新增的 behind 图像路径
                temp4 = temp.replace("IMG_LEFT", "POSE_TXT").replace('jpg', 'txt')
                if not (os.path.isfile(temp) and os.path.isfile(temp1) and os.path.isfile(temp2) and os.path.isfile(temp3) and os.path.isfile(temp4)):
                    continue
                # check data format
                pose_data = np.loadtxt(temp4)
    ...
    def __getitem__(self, item):
        ...
        img3_data = img3_data / 255.0
        img3_data = cv2.resize(img3_data, dsize=(self.width, self.height))
        img3_data = np.expand_dims(img3_data, axis=0)  # 添加通道维度，使其为4维

        # pose label
        temp4 = img_path_abs.replace("IMG_LEFT", "POSE_TXT")
        temp4 = temp4.replace('jpg','txt')
        joint_config, _ = self.PoseReader.run(temp4)
        ...
        #print("img3_tensor shape:", torch.Tensor([img3_data]).shape)        
        #print("joint_config shape:", torch.Tensor([joint_config]).shape)  
        #print("objectPose shape:", torch.Tensor(temp_label).shape)          
        return torch.Tensor([img1_data]), torch.Tensor([img2_data]), torch.Tensor([img3_data]), torch.Tensor(np.array([joint_config])), torch.from_numpy(temp_label)
