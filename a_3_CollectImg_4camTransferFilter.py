...
# 异常检测方法，使用Z-score过滤异常数据
def filter_pose_data(pose_data, z_threshold=2.5):
    pose_data_filtered = []
    count_removed = 0
    
    # 对每个列计算z-score
    z_scores = np.abs(zscore(pose_data[:, 1:]))  # 只计算位置和姿态部分，不包含ID
    mask = (z_scores < z_threshold).all(axis=1)  # 判断哪些行的数据是正常的
    count_removed = np.sum(~mask)  # 计算被剔除的个数
    pose_data_filtered = pose_data[mask]  # 保留正常数据
    
    return pose_data_filtered, count_removed
...
if __name__ == "__main__":
    CAP = MultiVideoCapture(cameraID_outside=8, cameraID_left=6, cameraID_right=4, cameraID_behind=10)#4/5
    
    # 使用指定文件夹名
    img_folder = '/home/sen/Documents/InHand_pose/IMG_DATA_LS/IMG_DATA_J2_250801'#1/5
    #img_folder = '/home/sen/Documents/InHand_pose/IMG_DATA_LS/IMG_DATA_MASTERBALL_0907test' + datetime.now().strftime("%m%d-%H%M%S")
    outside_folder = "IMG_OUTSIDE"
    left_folder = "IMG_LEFT"
    right_folder = "IMG_RIGHT"
    behind_folder = "IMG_BEHIND"
    result_folder = "POSE_TXT"
    raw_folder = "POSE_TXT_RAW"
    filtered_folder = "POSE_TXT_filtering"

    for folder in [outside_folder, left_folder, right_folder, behind_folder, result_folder, filtered_folder, raw_folder]:
        os.makedirs(os.path.join(img_folder, folder), exist_ok=True)

    MarkerDetector = ReadAruCo()
    initial_pose_path = "./IMG_DATA_LS/IMG_DATA_J2_0801-215707/Pose_Init"#2/5
    ...
    #for j in range(0,500): #h5, change Rx 
    #for j in range(500,1000): #h5, change Rz 
    #for j in range(1000,1500): #h4, change Rx 
    #for j in range(1500,2000): #h4, change Rz 
    #for j in range(2000,2500): #h3, change Rx 
    #for j in range(2500,3000): #h3, change Rz 
    #for j in range(3000,3500): #h2, change Rx
    #for j in range(3500,4000): #h2, change Rz 
    #for j in range(4000,4500): #h1, change Rx 
    #for j in range(4500,5000):  #h1, change Rz  #3/5
    #for j in range(0,1000): #h5, change Rx 
    #for j in range(1000,2000): #h4, change Rx 
    #for j in range(2000,3000): #h3, change Rx 
    #for j in range(3000,4000): #h2, change Rz 
    for j in range(4000,5000): #h1, change Rx 
        ...
        if pose_data:
            ...
            if all(req_id in detected_ids for req_id in required_ids):
                ...
                np.savetxt(os.path.join(img_folder, raw_folder, file_name_txt), result_pose)
     
                # 对pose_data进行过滤并保存到POSE_TXT_filtering文件夹
                filtered_data, count_removed = filter_pose_data(np.array(pose_data))
                valid_percentage = (len(filtered_data) / len(pose_data)) * 100
                print(f"Valid data percentage: {valid_percentage:.2f}%, Removed: {count_removed}")

                # 累计总数据量和有效数据量
                total_data_count += len(pose_data)
                total_valid_data_count += len(filtered_data)

                np.savetxt(os.path.join(img_folder, filtered_folder, file_name_txt), filtered_data)
