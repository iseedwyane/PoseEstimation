        ...
        try:
            while i < 1000:
                ...
                if result_pose is not False:
                    ...
                    # 使用生成的文件路径调用 run 方法
                    joint_config, GroundTruth_objectPose = a.run(temp_data_path)
                    print("Transformed joint_config:", joint_config)
                    # 将 GroundTruthPOSE 的姿态数据（旋转向量）从弧度转换为度
                    if not all(np.isnan(GroundTruth_objectPose)):
                        GroundTruth_objectPose[:3] = GroundTruth_objectPose[:3]  # 保持位置部分不变
                        GroundTruth_objectPose[3:] = np.rad2deg(GroundTruth_objectPose[3:])  # 将旋转向量部分转为度
                        print("Transformed GroundTruthPOSE:", GroundTruth_objectPose)
                    
                    griper_matrix = a.pose2matrix(gripper_pose)
