...
class RealTimePrediction:
    def __init__(self):
        self.pre_model_pth = "./weights/reconstruction_weight_PEACH_241227.pth"#3/5
        self.width = 320
        self.height = 320
        self.batch_size = 32
...
if __name__ == "__main__":
    ...
    CAP = MultiVideoCapture(cameraID_outside=10, cameraID_left=8, cameraID_right=0, cameraID_behind=2)#4/7
    saved_pth = ["./weights/pose_weight_vase_250108.pth"]#`1/7
       #saved_pth = ["./weights/pose_weight_baseball_0807.pth"]    
    weight_path = saved_pth[0]
    print(weight_path)  
    time.sleep(3)
    pre_model_pth = "./weights/reconstruction_weight_VASE_250107.pth"#2/7
    #pre_model_pth = "./weights/reconstruction_weight_LS_64_BaseballOnly.pth"
    ...
    file_path = "./IMG_DATA_LS/IMG_DATA_VASE_250107/Pose_Init"#5/7
    ...
    # 创建并打开CSV文件以追加模式写入数据
    output_directory = '/home/sen/Documents/InHand_pose/IMG_DATA_LS/IMG_DATA_VASE_250107/PredictionTrails/'#6/7
    # 确保文件夹存在，如果不存在则创建 
    os.makedirs(output_directory, exist_ok=True)  
    #csv_filename = '/home/sen/Documents/InHand_pose/IMG_DATA_LS/IMG_DATA_PEACH_241226/PredictionTrails_data' + datetime.now().strftime("%m%d-%H%M%S") + '.csv'
    csv_filename = os.path.join(output_directory, 'output_data' + datetime.now().strftime("%m%d-%H%M%S") + '.csv')  
        ...
        try:
            ...
            while i<1000:
                ...
                if result_pose is not False:
                    ...
                    GroundTruthPOSE = np.hstack((transfer_matrix[0:3,3]*1000, euler_angles))

                    print("GroundTruthPOSE:", (euler_angles[0]),(euler_angles[1]),(euler_angles[2]))
                ...
                if outputs.numel() > 1:  # 确保outputs有多个元素
                    second_value = outputs[0, 5].item()  # 取第一行的第6个元素
                    #print("Predicted POSE 5:", second_value)
                ...
                PWM_PD_value = -(ref - second_value)*kp
...
# 获取当前生成的CSV文件名
#csv_filename = 'output_data' + datetime.now().strftime("%m%d-%H%M%S") + '.csv'
...
# 提取时间列
time = pd.to_datetime(data['time'])  # 假设'time'列为时间
initial_time = time.iloc[0]
time_seconds = (time - initial_time).dt.total_seconds()  # 转换为相对的秒数

# 提取传感器数据列（明确指定列名）
sensor_columns = ['Sensoroutput1', 'Sensoroutput2', 'Sensoroutput3', 'output4', 'output5', 'output6']
sensor_outputs = data[sensor_columns]

# 打印sensor_outputs的形状和前几行，帮助调试
print("sensor_outputs shape:", sensor_outputs.shape)  
print(sensor_outputs.head())  # 打印前几行数据，检查是否有问题
...
for i, column in enumerate(sensor_columns):
    label = sensor_labels[column]  # 获取自定义的标签
    plt.plot(time_seconds, sensor_outputs[column], label=label, linewidth=1.5)
...
plt.ylim([min(sensor_outputs.min()), max(sensor_outputs.max())])  # 根据数据范围设置纵轴范围
...
# 定义文件夹路径
output_directory = '/home/sen/Documents/InHand_pose/IMG_DATA_LS/IMG_DATA_VASE_250107/PredictionTrails/'#7/7
...
filename = os.path.join(output_directory, f'output_figure_{timestamp}.png')

plt.savefig(filename, dpi=300) # 以300dpi保存图形为PNG格式文件
plt.show()