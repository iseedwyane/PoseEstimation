...
class AE(BaseModel):
    ...
    def __init__(self, in_channels=1, out_channels=1):
        ...
        self._decoder = nn.Sequential(
            OrderedDict(
                [
                    ...
                    (
                        "dec_conv5",
                        nn.Conv2d(self.hid_dims[0], out_channels, kernel_size=1, stride=1, bias=True),
                    ),
        ...
        # self._encoder_MPL = nn.Sequential(
        #     OrderedDict([
        #         ("enc_h1", nn.Linear(self.hid_dims[-1]*self.encoder_flat_size, self.latent_num),),
        #         ])
        # )

        # self._decoder_MPL = nn.Sequential(
        #     OrderedDict([
        #         ("denc_h1", torch.nn.Linear(self.latent_num, self.hid_dims[-1]*self.encoder_flat_size),),
        #         ])
        # )
    ...
    def forward(self, x):
        ...
        # out_dec = nn.functional.normalize(out_dec)
