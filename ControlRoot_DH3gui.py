    ...
    def sendCmd(self, ModbusHighAddress, ModbusLowAddress, Value=0x01, is_write=True, isReadSerial=True):
        ...
        if isReadSerial:
                for attempt in range(3):  # 最多重试3次
                    ...
                    print(f"收到的数据长度: {len(back)}")
                    print(f"收到的原始数据: {back.hex()}")

                    if len(back) >= 14:  # 确保接收到的数据长度足够
                        if ModbusHighAddress == 0x05:
                            grip_force = int.from_bytes(back[9:10], byteorder='big', signed=False)  # 解析实际的夹持力值
                            return grip_force
                        elif ModbusHighAddress == 0x06:
                            position_value = int.from_bytes(back[9:10], byteorder='big', signed=False)  # 解析位置值
                            return position_value
                        elif ModbusHighAddress == 0x07:
                            angle_value = int.from_bytes(back[9:10], byteorder='big', signed=False)  # 解析角度值
                            return angle_value
                        elif ModbusHighAddress == 0x0F:
                            feedback_value = int.from_bytes(back[9:10], byteorder='big', signed=False)  # 解析角度值
                            return feedback_value                       
                    else:
                        time.sleep(0.05)  # 等待一段时间后重试
                raise ValueError("收到的数据长度不足，无法解析")
        else:
            time.sleep(0.01)
            return