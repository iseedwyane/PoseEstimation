...
if __name__ == "__main__":
    ...
    img_folder = '/home/sen/Documents/InHand_pose/IMG_DATA_LS/IMG_DATA_J2_' + datetime.now().strftime("%m%d-%H%M%S")  # 2/3
        ...
        if type(result_pose) == bool:
            ...
            for pose in result_pose:
                ...
                if marker_id == 11:  # 假设ID为11的标记代表Left
                    ...
                    roll = pose[3]  # yaw是第五个分量（根据你描述应该是第6列，数组下标从0开始）                   
                    # 进行加1.57再取余3.14的操作
                    roll = (roll + 1.57) % 3.14                    
                    # 更新姿态数据
                    pose[4] = roll  # 更新yaw分量

                    pitch = pose[4]  # yaw是第五个分量（根据你描述应该是第6列，数组下标从0开始）                   
                    # 进行加1.57再取余3.14的操作
                    pitch = (pitch + 1.57) % 3.14                    
                    # 更新姿态数据
                    pose[4] = pitch  # 更新yaw分量
    ...
    img_folder = '/home/sen/Documents/InHand_pose/IMG_DATA_LS/IMG_DATA_J2_PlotPose_Init'  # 3/3
