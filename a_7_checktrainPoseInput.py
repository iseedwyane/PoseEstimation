...
class TrainRegression(object):
    ...
    def train(self):
        # load data
        # return torch.Tensor(np.array([img1_data])), torch.Tensor(np.array([img2_data])), torch.Tensor(np.array([joint_config])), torch.Tensor(np.array([objectPose]))
        ...
        print("sample dataset_image[3]'s len: ", len(dataset_image[3]))#torch.Tensor(np.array([objectPose]))
...
# test data,each components
    ...
    def checkInput(self):
        # 加载数据
        dataset_image = ReconstDataset(data_path=self.data_folder, scaled_width=self.width, scale_height=self.height)
        # 返回 torch.Tensor(np.array([img1_data])), torch.Tensor(np.array([img2_data])), torch.Tensor(np.array([joint_config])), torch.Tensor(np.array([objectPose]))
        print("sample numbers: ", len(dataset_image))
        print("sample dataset_image[3]'s len: ", len(dataset_image[3]))

        img1_data, img2_data, joint_config, objectPose = dataset_image[3]

        print("img1_data: ", img1_data)
        print("img2_data: ", img2_data)
        print("joint_config: ", joint_config)
        print("objectPose: ", objectPose)

        objectPose_tensor = objectPose.numpy()
        print("objectPose_tensor: ", objectPose_tensor)    
...
def plot_objectPose_distribution(data_folder, num_bins=50, folder_name=""):
    ...
    for i in range(len(dataset_image)):
        _, _, _, objectPose = dataset_image[i]
    ...
    for i in range(6):
        ...
        #plt.title(f'{folder_name} - Dime {i+1}')
        plt.xlabel(labels[i])
...
if __name__ == "__main__":
    ...
    for i in range(len(data_path)):
        ...
        #test.checkInput()
        # 调用 plot_objectPose_distribution 方法绘制直方图
