...
#begin in-hand manip & recoding
if __name__ == "__main__":
    # 定义目标位置
    home =  [-0.087456885062373, -0.6282124254282055, 0.5867273177266203, -0.2185600137599746, 3.1323994110754554, 0.011654112075736701]

    approach = [-0.08744172023417086, -0.6282270869485959, 0.4478657888310303, -0.21855277043854626, 3.1324432510823312, 0.01163866327081556]
   ...
   # robot.move(home, speed=0.05, acceleration=0.5, async_move=False)
    ...
    pick =  [-0.08744217030378179, -0.6282261571231937, 0.4357121285267734, -0.21857695537779417, 3.1324275522604865, 0.011655059065769073]
    #robot.move(pick, speed=0.05, acceleration=0.3, async_move=False)
    step=1

    gripper.Position(85)#1/1
    gripper.angle(60)
    gripper.Force(15)

    goup = [-0.08744118739739098, -0.6282276807190845, 0.6237597523355269, -0.21856070325557883, 3.1324484321255337, 0.011679185957335343]
    ...
    CAP = MultiVideoCapture(cameraID_outside=10, cameraID_left=0, cameraID_right=8, cameraID_behind=2)
    saved_pth = ["./weights/pose_weight_masterball_0923.pth", "./weights/pose_weight_peach_0912.pth", "./weights/pose_weight_cover_0912.pth"]
    ...
    file_path = "./IMG_DATA_LS/IMG_DATA_MASTERBALL_1001-173214/Pose_Init"#1
        ...
        try:
            ...
            while i<1000:
                ...
                if result_pose is not False:
                    ...
                    gripper_config = 0.1330280649243982 * 1000
...
y2 = data.iloc[:, 12] +250# 第九列数据

plt.figure()

plt.plot(time_seconds, refH, color=[0.5, 0.54, 0.53], linewidth=2, linestyle='--', label='Ref Pitch')
